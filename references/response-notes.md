...
- 뉴스 목록 카테고리: `MAINNEWS`, `FLASHNEWS`, `RANKNEWS`. CLI는 소문자 입력도 현재 UI의 대문자 값으로 정규화합니다.
- 시장 공시 뉴스는 현재 화면처럼 생략 시 오늘까지 최근 3개월을 `startDate`/`endDate`로 보냅니다.
...
- 이름에 `mystock`이 포함돼도 `/api/securityService/home/v3/mystock/ranking/{code}`는 공개 종목별 집계 랭킹으로 확인된 정확한 경로만 허용합니다. 그 밖의 `mystock` 경로는 계속 차단합니다.
