...
5. Run the local checks on the latest Python 3.14 patch release, the sole supported runtime. Use the Ruff version pinned in [CI](../.github/workflows/ci.yml) for reproducible results:
...
11. Before any tagged release, require all of the following:
    ...
    - compare the latest release tag with the release candidate; increment the major version when removing supported Python runtimes or breaking public CLI commands, options, accepted enums, defaults, or paging semantics. Record affected usage and migration steps in the changelog and release notes; compatibility shims are not required when a major release documents the break;
    - review `CHANGELOG.md` Unreleased entries against the candidate, distinguish repository compatibility from upstream API availability, and keep historical validation claims tied to the version actually tested;
    - in the release PR, move selected Unreleased entries under the planned version and KST release date, retain remaining unreleased work separately, and align README's release status;
    - a clean PR is reviewed and merged to `main` before creating the tag and release notes.

12. After the gate passes on merged `main`, confirm the changelog version and date match the release being published, create the planned semantic-version tag, and publish concise release notes. Do not raise the public version in advance of the merged, validated commit. Read back the published notes to verify links, Korean text, and real newlines; correcting an old release's wording must preserve its original scope and validation results.
