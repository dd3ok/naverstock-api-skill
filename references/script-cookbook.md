...
- `scripts/stock_detail_pages.py`: 종목 상세의 가격표, 호가, 차트 가격, 뉴스, 공시, IR 목록/상세, 리서치, 투자자 통계, ETF 상세 내부 데이터 조회.
...
- `scripts/marketindex.py`: 주요 지수 목록, 시장지표 주요 블록/카테고리, 환율 목록, 지수 폴링, 지수 차트 조회.
- `scripts/crypto.py`: 업비트/빗썸 랭킹, 주요 코인, 폴링 가격, 분봉 캔들, 코인 뉴스/프로필 조회.
- `scripts/news.py`: 시장 뉴스 목록, 뉴스포커스 하위 탭, 키워드 검색, 해외뉴스 목록/상세 조회.
- `scripts/research.py`: 리서치 카테고리별 목록, 랭킹, 최신/산업 블록, 최근 인기 리서치, 증권사 목록, 리포트 상세 조회.
- `scripts/discussion.py`: 읽기 전용 종목토론 feed, 시장 feed, 인기 목록, 글 상세, 이전/다음 글, 관련 인기 글, 종목 토론 랭킹 조회.
