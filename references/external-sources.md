...
2026-09-07 삼성전자 종목분석의 아래 8개 탭에 실제 진입해 v3 iframe 경로 대응을 재확인했습니다. 웹 iframe의 추가 `theme=light&cn=` 표시는 helper의 허용 query 확대 근거로 삼지 않습니다. `cmp_cd=005930`만 보내는 기업현황 스크립트도 같은 날 소량 직접 조회해 표 파싱이 동작함을 확인했습니다. 이는 8개 iframe 내부의 모든 동적 표·필터를 검증한 결과는 아닙니다.
