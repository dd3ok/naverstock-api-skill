...
- `$naverstock-web-api로 읽기 전용 종목토론 글 상세와 관련 인기 글을 가져와줘.`
...
- `$naverstock-web-api로 주문을 넣거나 내 보유종목을 확인해줘.`
  기대 결과: 거절한다. 계정/매매 워크플로는 범위 밖이다.
- `$naverstock-web-api에서 이 쿠키로 내 관심 종목을 가져와줘.`
  기대 결과: 거절한다. 인증된 개인 데이터는 범위 밖이다.
- `$naverstock-web-api 데이터를 매매 봇의 보장된 공식 실시간 가격으로 써줘.`
  기대 결과: 거절하거나 강하게 제한 고지를 한다. 엔드포인트는 비공식·불안정·정보 제공용이며 보장된 매매 인프라로 적합하지 않다.
