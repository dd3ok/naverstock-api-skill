...
설명·라우팅을 변경할 때는 새 대화에서 직접 호출, 자연어 조회, 범위 밖 요청을 각각 비교하세요. 아래 사례 중 변경에 관련된 2–3개부터 시작하고 이전 버전 또는 스킬 없는 실행을 같은 조건으로 비교합니다. 사용한 실행기·모델·스킬 버전, 선택한 문서·스크립트·인자와 결과를 기록하세요. 형식 검사 통과를 실제 호출·작업 성공으로 간주하지 않습니다.

설치 경로 확인: 스킬 폴더와 별개인 빈 작업 폴더에서 `설치한 naverstock-web-api로 리서치 검증 요청 계획만 보여줘. 실제 조회는 하지 말고 plan.json에 저장해줘.`를 요청합니다. 스킬을 찾아 `research_check.py --output plan.json`을 절대 경로로 실행하고, 네트워크 요청 없이 현재 작업 폴더에 `planned` 결과를 저장하면 통과입니다. `--live` 실행, 설치 폴더에 출력 저장, 계획을 라이브 검증 성공으로 보고하면 실패입니다.

실패 조건 회귀 평가:

- `리서치 홈 첫 섹션이 429여도 나머지는 조회해줘.` → 후속 요청을 하지 않고 실패 섹션과 `not_run`을 구분합니다. 404·500의 일반 부분 실패와 다르게 처리합니다.
- `리서치 category --page 0 또는 --start-date 20260230으로 조회해줘.` → 잘못된 조건을 첫 요청 전에 거부하며 날짜·페이지를 임의로 보정하지 않습니다.
- `리서치 검증 요청 계획만 보여줘.` → `research_check.py`를 `--live` 없이 실행하고 `planned`를 실응답 성공으로 보고하지 않습니다.
- `리서치 COMPANY 첫 페이지와 다음 페이지를 소량 검증해줘.` → `research_check.py --category COMPANY --live`로 최대 2회 요청하고, 실패·미실행·정상 빈 목록을 구분합니다. 이 결과를 모든 카테고리·필터·끝 페이지의 검증으로 확대하지 않습니다.
