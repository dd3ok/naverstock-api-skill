...
gemini skills link "$(pwd)/naverstock-api-skill"
```

이미 clone한 폴더가 있다면 해당 경로를 바로 연결하세요.

```bash
gemini skills link /path/to/naverstock-api-skill
```
...
| `scripts/market_trend.py` | 투자자 예탁금, 시장 집계 투자자 동향, 외국인/기관 투자자 동향 랭킹 |
| `scripts/marketindex.py` | 지수, 원자재, 운임, 금리, 환율, 경제지표 |
...
| `scripts/discussion.py` | 읽기 전용 종목토론 글, 인기 글, 종목 토론 랭킹 |
