...
이 스킬은 네이버 증권, 네이버페이 증권, Npay 증권으로 불리는 공개 `stock.naver.com` 화면에서 확인되는 비공식 읽기 전용 내부 API를 점검하고 호출할 때 사용합니다. 국내·해외 주식, 시세, 지수, 가상자산, 통합 검색, 홈 콘텐츠, 뉴스, 리서치, 랭킹, IPO, ETF, 시장지표, 종목토론 데이터를 다룹니다. 현재 종목 페이지가 연결하는 WiseReport v3 기업분석과 신버전에 없는 제한된 기술적 조건검색만 명시적인 외부 소스로 보완합니다.

## 핵심 안전 규칙

- 공개 `stock.naver.com/api/...`에서 관찰되는 비공식·미문서화 read-only 엔드포인트를 우선합니다. 네이버, 네이버페이, 네이버파이낸셜 또는 증권사가 지원하는 공개 API처럼 표현하지 않습니다.
...
- 일반적인 누락 데이터를 `finance.naver.com` HTML로 대체하지 않습니다. 예외는 `scripts/legacy_screeners.py`가 allowlist로 고정한 기술적 5종과 가격 위치 2종뿐이며, WiseReport는 `scripts/wisereport.py`만 사용합니다. 자세한 경계는 [references/external-sources.md](references/external-sources.md)를 따릅니다.
...
404를 자동으로 "자료 없음"이나 빈 목록으로 바꾸지 않습니다. 단일 조회는 구조화된 API 오류로 실패시키고, 여러 독립 섹션을 모으는 `research.py home`만 실패 섹션을 `unavailable`로 표시한 뒤 나머지 조회를 계속합니다.
...
| 국내 시장 랭킹, KONEX 거래량, 관리/정지/투자경고, 배당, IPO, 업종/테마 랭킹 | `scripts/market_stock.py` | [references/api-catalog.md](references/api-catalog.md) |
...
| 리서치 v2 카테고리/상세/종목별 목록, 랭킹, 최신/주간 인기, 증권사 목록, best-effort 홈과 명시적 v1 호환 조회 | `scripts/research.py` | [references/api-catalog.md](references/api-catalog.md) |
...
3. 공지는 `stockSecurity/notices/v2`, 리서치는 `stockSecurity/researches/v2` 계열을 우선합니다. `research.py v1-*` 명령은 명시적 호환 조회에만 사용합니다.
4. 기업분석 8종은 현재 종목 페이지가 연결하는 `wisereport.py` v3를 사용합니다. 일반 시세·뉴스·리서치를 이 소스로 대체하지 않습니다.
5. `legacy_screeners.py`는 사용자가 해당 조건검색을 요청했을 때만 사용합니다. 기술적 명령에는 시장 인자를 붙이지 않고, 가격 위치 명령에서만 KOSPI/KOSDAQ을 고릅니다.
6. 스크립트가 감싸지 않은 엔드포인트 계열은 호출 전 [references/api-catalog.md](references/api-catalog.md)를 읽고 `script-backed`, `observed`, `needs-recheck`, `excluded` 상태를 확인합니다.
